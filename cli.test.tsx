...
test('should render retrying status with timer and attempt info', () => {
  ...
  expect(lastFrame()).toContain('Time elapsed: 0s');
  expect(lastFrame()).toContain('Previous attempt failed. Retrying... (1 failed attempts)');
});

test('should render retrying status without extra info when props missing', () => {
  const { lastFrame } = render(<Cli status="retrying" />);

  expect(lastFrame()).toContain('Retrying commit message generation (attempt undefined/undefined)...');
  expect(lastFrame()).not.toContain('Previous attempt failed');
});
...
test('should handle status transitions including retry', () => {
  ...
  rerender(<Cli status="retrying" attempt={2} maxAttempts={3} />);
  expect(lastFrame()).toContain('Retrying commit message generation (attempt 2/3)...');
  expect(lastFrame()).toContain('Previous attempt failed. Retrying... (1 failed attempts)');
