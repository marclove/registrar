...
afterEach(() => {
  ...
  // Explicitly restore the original node:fs module
...
test("loadConfig should handle invalid provider in config", async () => {
  ...
  expect(mockConsoleError).toHaveBeenCalledWith(
    'Invalid provider "invalid" in config.toml. Must be one of: anthropic, openai, google. Falling back to default provider "anthropic"',
  );
});
